...
from parrots.indextts.gpt.conformer_encoder import ConformerEncoder
...
class UnifiedVoice(nn.Module):
    def __init__(
            self, layers=8, model_dim=512, heads=8, max_text_tokens=120, max_mel_tokens=250, max_conditioning_inputs=1,
    ...
    def forward(self, speech_conditioning_latent, text_inputs, text_lengths, mel_codes, mel_codes_lengths,
        ...
        if emo_vec is None:
            emo_vec_syn_ori = self.get_emo_conditioning(emo_speech_conditioning_latent.transpose(1, 2),
        ...
        conds = torch.cat((speech_conditioning_latent + emo_vec.unsqueeze(1), duration_emb_half.unsqueeze(1),
        ...
        text_logits, mel_logits = self.get_logits(conds, text_emb, self.text_head, mel_emb, self.mel_head,
                                                  get_attns=False, return_latent=True)
        return mel_logits[:,
    ...
    def prepare_gpt_inputs(
        ...
        for i in range(b):
            ...
            if padding > 0:
                pad = torch.zeros((padding, conditional_latents.size(-1)), dtype=text_emb.dtype,
                                  device=device)  # [p, dim]
    ...
    def inference_speech(self, speech_condition, text_inputs, emo_speech_condition=None, cond_lengths=None,
        ...
        conds_latent = torch.cat((speech_conditioning_latent + emo_vec.unsqueeze(1), duration_emb_half.unsqueeze(1),
                                  duration_emb.unsqueeze(1)), 1)
    ...
    def merge_emovec(self, speech_conditioning_latent, emo_speech_conditioning_latent, cond_lengths, emo_cond_lengths,
                     alpha=1.0):
